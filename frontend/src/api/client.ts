...
export const api = {
  ...
  listBackupRecords: (taskId?: number) =>
    request<any[]>(`/backup-records${taskId ? `?task_id=${taskId}` : ""}`),
  ...
  listMigrationRecords: (taskId?: number) =>
    request<any[]>(`/migration-records${taskId ? `?task_id=${taskId}` : ""}`),
};
